...
sealed interface AppwriteResult<out T> {
    ...
    companion object {
        ...
        inline fun <T> catching(block: () -> T): AppwriteResult<T> =
            try {
                Success(block())
 ...
 */
inline fun <T, R> AppwriteResult<T>.map(transform: (T) -> R): AppwriteResult<R> =
    when (this) {
